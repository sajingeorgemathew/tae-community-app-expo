...
type Props = NativeStackScreenProps<MoreStackParamList, "AdminInstructors">;
...
export default function AdminInstructorsScreen({ navigation }: Props) {
    ...
    if (tutorIds.length > 0) {
      ...
      if (assignments) {
        ...
        for (const a of assignments) {
          counts[a.tutor_id] = (counts[a.tutor_id] || 0) + 1;
        }
  ...
  // Re-fetch when coming back from detail
  useEffect(() => {
    const unsub = navigation.addListener("focus", () => {
      if (!loading) fetchInstructors();
    });
    return unsub;
  }, [navigation, loading, fetchInstructors]);
  ...
  if (error && profiles.length === 0) {
    return (
      <View style={styles.center}>
        ...
        <Pressable style={styles.retryButton} onPress={fetchInstructors}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <InstructorRow
            ...
            onPress={() =>
              navigation.navigate("AdminInstructorDetail", {
                profileId: item.id,
              })
            }
          />
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 15, fontWeight: "600", color: "#111", marginBottom: 4 },
  metaRow: {
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  ...
  listedTag: { fontSize: 11, color: "#047857" },
  unlistedTag: { fontSize: 11, color: "#b45309" },
  courseCount: { fontSize: 11, color: "#666" },
